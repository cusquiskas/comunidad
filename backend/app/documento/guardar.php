<?php
    ...
    if ($_POST['doc_movimiento'] == "" && $_POST['doc_split'] == "") {
    ...
    $preNombre = ($_POST['doc_split'] == "")
                 ?"m".str_pad($_POST['doc_movimiento'], 8, "0", STR_PAD_LEFT)
                 :"s".str_pad($_POST['doc_split'     ], 8, "0", STR_PAD_LEFT);
    ...
    if (isset($_FILES['doc_real']) && $_FILES['doc_real']['error'] == 0) {
        ...
        $nombre    = str_pad($_POST['doc_comunidad'], 4, "0", STR_PAD_LEFT) . "_" . $preNombre;# . "." . $extension;
        ...
        if ($_POST['doc_split'] == "") {
            ...
            $split = ["spl_split"=>$_POST['doc_split'], "spl_documento"=>$docu["doc_documento"]];
            $manSplit->save($split);
            $split = $manSplit->getListaErrores();
            if ($movi != null && count($split) > 0 ) {
                unset($manSplit);
                die(json_encode(['success' => false, 'root' => $split]));
